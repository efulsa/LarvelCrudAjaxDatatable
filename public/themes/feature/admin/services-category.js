$(function () {
    $("#dtTableServicesCategory").DataTable({
        responsive : true,
        autoWidth  : false,
        processing : true,
        searching  : true,
        ...
        ajax: function (data, callback) {
            $.ajax({
                url       : '/admin/servicesCategoryDataTables',
                data      : data,
                dataType  : 'json',
                beforeSend: function () {
                    $('#dtTableServicesCategory > tbody').block({
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                ...
                type: 'POST',
                url : "/admin/deleted/services-category",
                data: {
                    id: idx
                },
                success: function (data) {
                    $('#dtTableServicesCategory').DataTable().draw();
                    setTimeout((function () {
                        toastr.options = {
...
$('#sample_form').on('submit', function (event) {
    ...
    if ($('#action').val() == 'Add') {
        action_url = "/admin/store/services-category";
        toastText  = 'ditambahkan';
    }

    if ($('#action').val() == 'Edit') {
        action_url = "/admin/update/services-category";
...
$(document).on('click', '.edit', function () {
    ...
    $('#form_result').html('');
    $.ajax({
        url     : "/admin/services-category/" + id,
        dataType: "json",
        success : function (data) {
